...
const PrintInventory = forwardRef(({ items, businessName }, ref) => {
  // 1. SURGICAL FIX: Calculate totals safely
  const totalStock = items.reduce((sum, it) => sum + (Number(it.stock) || 0), 0);
  const totalValue = items.reduce((sum, it) => sum + ((Number(it.price) || 0) * (Number(it.stock) || 0)), 0);
  ...
  return (
    ...
    >
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {items.map((it, idx) => {
          // 🌟 SURGICAL FIX: Force item to show as Inactive if stock is 0
          const isAvailable = it.active && Number(it.stock) > 0;
          ...
          {/* 2. SURGICAL FIX: A clean totals row matching your existing layout */}
          <tr className="bg-gray-100 border-t-2 border-gray-200 font-bold">
            <td className="p-2 text-right text-gray-600 uppercase text-xs" colSpan="3">
              Total Inventory Value:
            </td>
            <td className="p-2 text-center text-[#0046A5]">
