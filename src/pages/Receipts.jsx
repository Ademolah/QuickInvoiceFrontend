...
export default function Receipts() {
  ...
  // 2. Localized currency configuration map
  ...
  // 3. Record-aware multi-currency formatter
  const formatReceiptCurrency = (amount, recordCurrency) => {
    const targetCode = recordCurrency && currenciesConfig[recordCurrency] ? recordCurrency : code;
    const target = currenciesConfig[targetCode];
          ...
          <div className="flex items-center justify-between md:justify-end gap-4 mt-2 md:mt-0">
            <div className="text-right">
              <span className="text-sm md:text-2xl font-black text-slate-900 tracking-tighter">
                {/* 🌟 SURGICAL FIX: Passing rows custom currency identifier token */}
                {formatReceiptCurrency(Number(inv.total), inv.currency)}
              </span>
            </div>
